...
- [ ] 0.1 Visit `https://notebooklm.google.com/` and document the notebook list DOM structure.
...
- [ ] 0.2 Identify the best injection point for folder UI on NotebookLM.
...
- [ ] 1.1 Add `FOLDER_DATA_NOTEBOOKLM` to `StorageKeys` in `src/core/types/common.ts`.
  - **Verify**: `bun run typecheck` passes.

- [ ] 1.2 Add storage key default to `StorageService` if needed (follow the AI Studio pattern).
  - **Verify**: `bun run typecheck` passes.

## 2. Manifest & Entry Point

- [ ] 2.1 Add `https://notebooklm.google.com/*` to `host_permissions` in `manifest.json` and `manifest.dev.json`.
  - **Verify**: `bun run build:chrome` succeeds.

- [ ] 2.2 Add `https://notebooklm.google.com/*` to `content_scripts.matches` in `manifest.json` and `manifest.dev.json`.
  - **Verify**: `bun run build:chrome` succeeds.

- [ ] 2.3 Add `notebooklm.google.com` hostname branch in `src/pages/content/index.tsx`.
...
- [ ] 3.1 Create `src/pages/content/folder/notebooklm.ts` with minimal `NotebookLMFolderManager` class.
  - Include: `init()`, `destroy()`, `load()`, `save()` lifecycle methods.
...
- [ ] 3.2 Write `vitest` unit test for `NotebookLMFolderManager` data load/save isolation.
  - Test: Loading data with NotebookLM storage key does not interfere with Gemini or AI Studio keys.
  - **Verify**: `bun run test` passes.

- [ ] 3.3 Create `startNotebookLMFolderManager()` export function in `notebooklm.ts`.
  - Wire into `index.tsx` (replacing the stub from 2.3).
  - **Verify**: `bun run build:chrome` succeeds.

## 4. DOM Integration (selectors from Task 0)

- [ ] 4.1 Implement `waitForNotebookList()` — wait for the notebook list container using selectors from DOM research.
  - **Verify**: Unit test with JSDOM mock of the container element.

- [ ] 4.2 Implement `extractNotebookId(element)` — extract unique notebook ID from a notebook card element.
  - **Verify**: Unit test with mock DOM elements matching the real selectors found in Task 0.

- [ ] 4.3 Implement `injectFolderUI()` — create and insert the folder container at the injection point from Task 0.
  - Include: folder header with "Folders" title and "Add Folder" button.
  - Include: folder list rendering using `this.data`.
  - **Verify**: Unit test verifying DOM structure of rendered output.
...
- [ ] 5.2 Implement `toggleFolder()` (expand/collapse).
  - **Verify**: Unit test verifying `isExpanded` state toggle and DOM re-render.

## 6. Drag & Drop

- [ ] 6.1 Implement `makeNotebooksDraggable()` — attach `dragstart` handlers to notebook cards.
...
- [ ] 6.2 Implement folder drop zone — attach `dragover`/`drop` handlers to folder elements.
...
- [ ] 6.3 Implement "remove from folder" (drag back to root or remove button).
  - **Verify**: Unit test verifying item removal from `folderContents`.

## 7. CSS Styling

- [ ] 7.1 Add NotebookLM-specific CSS rules to `public/contentStyle.css`.
...
- [ ] 8.1 Manual E2E: Load extension on Chrome, visit `notebooklm.google.com`, verify folder UI appears.
- [ ] 8.2 Manual E2E: Create a folder, drag a notebook into it, rename the folder, delete the folder.
- [ ] 8.3 Manual E2E: Verify Gemini folder functionality is unaffected (no regression).
- [ ] 8.4 Manual E2E: Verify AI Studio folder functionality is unaffected (no regression).
- [ ] 8.5 Run full verification suite: `bun run typecheck && bun run lint && bun run test && bun run build:chrome`.

## 9. i18n (if new keys needed)

- [ ] 9.1 If any new i18n keys are introduced, add translations to all 10 locales in `src/locales/*/messages.json`.
